...
(defun install-http (path)
  ...
  (let ((url (format nil "http://~a" (namestring path)))
    ...
    (download url txt)
    (build txt)
    (format t "~a -> ~a~%" (e txt) (r (pathname-name txt)))
    (rename-file (e txt) (r (pathname-name txt)))
...
(defun install-file (path)
  (unless (probe-file path)
    (error "not found: ~a" path))
  (build (pathname path))
  (format t "~a -> ~a~%" (e (pathname path)) (r (pathname-name path)))
  (rename-file (e (pathname path)) (r (pathname-name path))))
